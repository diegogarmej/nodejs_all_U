...
import { DoctorCreationError,  PatientGetAllError,  RecordNotFoundError,AppointmentUpdateError,AppointmentDeleteError, GetAllError} from "../../../config/customErrors"
...
export class AppointmentRepository {
    ...
    public async updateAppointment(id: number, updates: Partial<Appointment>): Promise<any> {
        try{
            const appointment = await db('citas').where({ id_cita: id }).update(updates)
            return appointment
        } catch (error){
